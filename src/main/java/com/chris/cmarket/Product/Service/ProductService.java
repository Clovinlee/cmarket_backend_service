...
import com.chris.cmarket.Order.Event.RevertPlaceOrderEvent;
...
public class ProductService {
    ...
    @Transactional
    @KafkaListener(topics = RevertPlaceOrderEvent.TOPIC_NAME, groupId = "revert_order_group-1")
    public void revertPlaceOder(RevertPlaceOrderEvent revertPlaceOrderEvent) {
        PlaceOrderEvent initialPlaceOrderEvent = revertPlaceOrderEvent.getPlaceOrderEvent();
        String slug = initialPlaceOrderEvent.getSlug();

        ProductModel product = this.productRepository.findBySlugForUpdate(slug).orElseThrow(
                () -> new NotFoundException("Product " + slug + " not found"));

        this.incrementStock(product, initialPlaceOrderEvent.getQuantity());
        log.info("Reverted order from kafka event: {}", revertPlaceOrderEvent);
    }
    ...
    @Transactional
    public void incrementStock(ProductModel product, int quantity) {
        product.setQuantity(product.getQuantity() + quantity);
        productRepository.save(product);
    }
